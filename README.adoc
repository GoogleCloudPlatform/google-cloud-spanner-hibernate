...
By default, this Spanner hibernate dialect batch the statements that it sends to the Cloud Spanner JDBC driver, with a batch size of 15. Additionally, inserts and updates will be ordered before execution

To disable batching or change these defaults, set `hibernate.jdbc.batch_size`, `hibernate.order_inserts`, and `hibernate.order_updates` in your Hibernate configuration file:
