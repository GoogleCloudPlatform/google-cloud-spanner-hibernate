...
| Constraint-based Annotations | The dialect does not support the database constraint-based annotations `@Column(unique = true)` and `@OnDelete(action = OnDeleteAction.CASCADE)`.
...
NOTE: Cloud Spanner does not support foreign key constraints, so foreign-key-columns are just regular columns in Cloud Spanner.
...
At the present time, the dialect does not support declaring unique columns by using the `@Column(unique = true)` annotation, but this feature will be added in the future.
