...
Cloud Spanner offers the concept of https://cloud.google.com/spanner/docs/schema-and-data-model#creating-interleaved-tables[Interleaved Tables] which allows you to cluster the rows of an interleaved table with rows of a parent table.
...
The primary key of the interleaved table must also include at least all of the primary key attributes of the parent and be named the same.
...
Note that the child's primary key is an `EmbeddedId` and also includes the primary key of the parent; this must be done for interleaved child entities.
