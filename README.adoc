...
separate sequence per entity. This generator is included in the Cloud Spanner Hibernate dialect.

The `increment_size` for the bit-reversed sequence can not exceed 200.
...
public class Employee {
  ...
  @GenericGenerator(
    ...
    parameters = {
      // Use a separate sequence for each entity.
