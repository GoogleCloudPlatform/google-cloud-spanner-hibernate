...
| Schema Constraints  | Dialect schema generation does not generate foreign key and column uniqueness constraints for entity relationships and columns. Server-side cascading deletes are also not supported.
...
At this time, the dialect does not support declaring unique columns by using the `@Column(unique = true)` annotation.
