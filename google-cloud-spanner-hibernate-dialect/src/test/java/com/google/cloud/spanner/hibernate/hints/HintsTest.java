...
public class HintsTest {
  ...
  public void testIndexStrategyFrom() {
    assertEquals("{\n"
        ...
        + "}",
        Hints
            .indexStrategyFrom("singers", IndexStrategy.FORCE_INDEX_UNION, ReplaceMode.ALL)
            .toComment());
  }
  ...
  public void testIndexStrategyJoin() {
    assertEquals("{\n"
        ...
        + "}",
        Hints
            .indexStrategyJoin("singers", IndexStrategy.FORCE_INDEX_UNION, ReplaceMode.ALL)
            .toComment());
  }
  ...
  public void testUseAdditionalParallelism() {
    assertEquals("@{USE_ADDITIONAL_PARALLELISM=true}select * from singers",
        Hints.useAdditionalParallelism(true).replace("select * from singers"));
    assertEquals("@{USE_ADDITIONAL_PARALLELISM=true}SELECT * from singers",
        Hints.useAdditionalParallelism(true).replace("SELECT * from singers"));
    assertEquals("@{USE_ADDITIONAL_PARALLELISM=false}"
  ...
  public void testOptimizerVersion() {
    assertEquals("@{OPTIMIZER_VERSION=1}select * from singers",
        Hints.optimizerVersion("1").replace("select * from singers"));
    assertEquals("@{OPTIMIZER_VERSION=latest_version}SELECT * from singers",
        Hints.optimizerVersion("latest_version").replace("SELECT * from singers"));
    assertEquals("@{OPTIMIZER_VERSION=default_version}"
            + "insert into singers (id, value) SELECT * from singers",
  ...
  public void testOptimizerStatisticsPackage() {
    assertEquals("@{OPTIMIZER_STATISTICS_PACKAGE=2024_02_27_10_00_00}select * from singers",
        Hints.optimizerStatisticsPackage("2024_02_27_10_00_00").replace("select * from singers"));
    assertEquals("@{OPTIMIZER_STATISTICS_PACKAGE=2024_02_27_10_00_00}SELECT * from singers",
        Hints.optimizerStatisticsPackage("2024_02_27_10_00_00").replace("SELECT * from singers"));
    assertEquals("@{OPTIMIZER_STATISTICS_PACKAGE=latest}"
            + "insert into singers (id, value) SELECT * from singers",
  ...
  public void testAllowDistributedMerge() {
    assertEquals("@{ALLOW_DISTRIBUTED_MERGE=true}select * from singers",
        Hints.allowDistributedMerge(true).replace("select * from singers"));
    assertEquals("@{ALLOW_DISTRIBUTED_MERGE=true}SELECT * from singers",
        Hints.allowDistributedMerge(true).replace("SELECT * from singers"));
    assertEquals("@{ALLOW_DISTRIBUTED_MERGE=false}"
            + "insert into singers (id, value) SELECT * from singers",
  ...
  public void testLockScannedRanges() {
    assertEquals("@{LOCK_SCANNED_RANGES=EXCLUSIVE}select * from singers",
        Hints.lockScannedRanges(LockScannedRanges.EXCLUSIVE).replace("select * from singers"));
    assertEquals("@{LOCK_SCANNED_RANGES=EXCLUSIVE}SELECT * from singers",
        Hints.lockScannedRanges(LockScannedRanges.EXCLUSIVE).replace("SELECT * from singers"));
    assertEquals("@{LOCK_SCANNED_RANGES=SHARED}"
            + "insert into singers (id, value) SELECT * from singers",
  ...
  public void testScanMethod() {
    assertEquals("@{SCAN_METHOD=ROW}select * from singers",
        Hints.scanMethod(ScanMethod.ROW).replace("select * from singers"));
    assertEquals("@{SCAN_METHOD=BATCH}SELECT * from singers",
        Hints.scanMethod(ScanMethod.BATCH).replace("SELECT * from singers"));
    assertEquals("@{SCAN_METHOD=AUTO}"
            + "insert into singers (id, value) SELECT * from singers",
  ...
  public void testPreferStreamable() {
    assertEquals("select@{PREFER_STREAMABLE=true} * from singers",
        Hints.preferStreamable(true).replace("select * from singers"));
    assertEquals("SELECT@{PREFER_STREAMABLE=false} * from singers",
